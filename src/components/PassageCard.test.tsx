...
describe("PassageCard", () => {
  it("renders no stamp before a verdict is submitted", () => {
    render(<PassageCard passage={passage} position={1} total={10} tilt={null} outcome={null} />);
    expect(screen.queryByText("CORRECT")).not.toBeInTheDocument();
    expect(screen.queryByText("WRONG")).not.toBeInTheDocument();
  });

  it("stamps CORRECT without shaking on a right guess", () => {
    render(<PassageCard passage={passage} position={1} total={10} tilt="human" outcome="correct" />);
  ...
  it("stamps WRONG and shakes the card on a missed guess", () => {
    render(<PassageCard passage={passage} position={1} total={10} tilt="ai" outcome="wrong" />);
